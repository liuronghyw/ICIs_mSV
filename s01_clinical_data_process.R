...
setwd("D:/analysis_lab/pharmacomicrobiomics/1_ICB_metagenome_SNV/pipeline/4_R/00.rawData")
source("D:/analysis_lab/pharmacomicrobiomics/1_ICB_metagenome_SNV/pipeline/4_R/functions.R")
...
clin_541981$study<-"PetersBA_2020"
...
#############################################################################################################
### 合并临床信息 (399742除了response，缺乏基本的临床资料)

clinical_sub<-rbind(clin_770295_sub,clin_22863_sub,clin_762360_sub,clin_541981_sub,
clin_397906_sub,clin_751792_sub)
...
for(i in 1:nr){
  if(is.na(clinical_whole$os[i])==T){clinical_whole$os_1_year[i]=NA}
  else if (is.na(clinical_whole$os_event[i])==T){clinical_whole$os_1_year[i]=NA}
...
nr<-nrow(clinical_whole)
for(i in 1:nr){
  if(is.na(clinical_whole$os[i])==T){clinical_whole$os_2_year[i]=NA}
  else if (is.na(clinical_whole$os_event[i])==T){clinical_whole$os_2_year[i]=NA}
  else if (is.na(clinical_whole$os[i])==F & is.na(clinical_whole$os_event[i])==F){
...
clinical_whole_reads$gender_num<-as.factor(clinical_whole_reads$gender) %>% as.numeric

write.csv(clinical_whole_reads,"clinical/clinical_whole_reads.csv",row.names=F)
write.table(clinical_whole_reads, "D:/analysis_lab/pharmacomicrobiomics/1_ICB_metagenome_SNV/pipeline/4_R/01.cleanData/phen/Clinical_basic.tsv",sep = '\t')
