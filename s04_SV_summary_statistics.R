...
#pdf("pics/FS1_Informative_species_number.pdf", height = 3.5, width = 3.5)
#print(p_species_n)
#dev.off()
...
tiff(file = "pics/F2_Total_SVs_number.tiff", width =2300, height =2300, res =300) 
print(p_pie)
dev.off()

tiff(file = "pics/F2_Total_SVs_number_nolabel.tiff", width =2300, height =2300, res =300) 
print(p_pie_nolabel)
dev.off()
...
p_sv_species<-ggplot(species_sgv_n_long, aes(x=Short_name, y=number, group = type, fill = type))+
  ...
  theme(plot.subtitle = element_text(vjust =1), 
        ...
        legend.position="top",
        legend.key = element_rect(fill = NA), 
        legend.text = element_text(size =15,family ="sans",color="black"),
...
tiff(file = "pics/F2_SVs_number_species.tiff", width =2600, height =1100, res =300) 
print(p_sv_species)
dev.off()
...
infor_sample_n <- info[,c(10,seq(15,21))]

infor_sample_n.long <- gather(infor_sample_n,'Cohort', 'Sample_size', c(2:8))
...
p_sample_n<-ggplot(infor_sample_n.long, aes(x=Short_name, y=Sample_size,group = Cohort))+
  ...
  scale_y_continuous(expand = c(0,0))+
  scale_fill_manual(name=NULL,
                      breaks = c("PRJEB22863_sample_number", "PRJNA397906_sample_number",
                              "PRJNA541981_sample_number","PRJNA751792_sample_number",
                              "PRJNA762360_sample_number","PRJNA770295_sample_number","PRJEB43119_sample_number"),
                      labels = c("RoutyB_2018", "FrankelAE_2017","PetersBA_2020","DerosaL_2022","McCullochJA_2022",
...
tiff(file = "pics/FS1_Samples_number_species.tiff", width =2600, height =1100, res =300) 
print(p_sample_n)
dev.off()
...
species_sample_n<-info[,c(1,10,15,11:14,22,23)]
...
p_sv_abun_density<-ggplot(sv_species_total_abun,aes(x=Total_abundance), color = "#2EC4B6")+
  geom_density(alpha = 0.2, color = "#b5182b", fill = "#34a186", alpha = 0.5)+
...
tiff(file = "pics/FS1_sv_abun_density.tiff", width =1000, height =800, res =300) 
print(p_sv_abun_density)
dev.off()
