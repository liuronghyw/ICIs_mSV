...
p_all_sv_pcoa<-ggplot(all_sv_avg_dist_pcoa,aes(X1,X2, color = all_basic_sv_rmna$dataset))+
  ...
  scale_color_manual(name=NULL, 
                    ...
                    labels = c("RoutyB_2018", "FrankelAE_2017","PetersBA_2020","DerosaL_2022",
                               "McCullochJA_2022","SpencerCN_2021","LeeKA_2022"),
                    values = mycolor7)+
...
tiff(file = "pics/F3C_all_SV_PCoA.tiff", width =1500, height =1500, res =300) 
print(p_all_sv_pcoa)
dev.off()
...
tiff(file = "pics/FS2_all_abun_PCoA.tiff", width =1500, height =1500, res =300) 
print(p_all_abun_pcoa)
dev.off()
...
all_inter_abun_pc_basic<-cbind(all_inter_abun_pc,all_inter_basic[,c("gender_num","age_bins","read_count","study")])

##colnames(all_inter_abun_pc_basic)[6]<-"gender"  （自变量中不允许出现NA）

all_sv_dist.adonis<-adonis(as.dist(all_inter_sv_dist)~X1+X2+X3+X4+X5+gender_num+age_bins+read_count+study, data = all_inter_abun_pc_basic)
...
all_basic_sv_p_adonis.df<-as.data.frame(matrix(NA, nrow = 9, ncol = 5))
demo_factors<-c( "read_count","gender_num","age_bins","X1","X2","X3","X4","X5","study")
...
all_basic_sv_p.adonis.comb<-adonis(as.dist(all_inter_sv_dist)~read_count+gender_num+age_bins+X1+X2+X3+X4+X5+study,data = all_inter_abun_pc_basic)
all_basic_sv_p.adonis.comb.mat<-all_basic_sv_p.adonis.comb$aov.tab[c(1:9),] %>%
...
all_basic_sv_p_adonis.df$Factor[2:9]<-c( "Gender","Age","abun_PC1","abun_PC2","abun_PC3","abun_PC4","abun_PC5","Cohort")
...
### 3.2 Visulization
demo_factors<-c("read_count","Gender","Age","abun_PC1", "abun_PC2","abun_PC3","abun_PC4","abun_PC5","Cohort")
...
#write.csv(msv_pc_cum0.6,"msv_pc_cum0.6.csv")
...
#write.csv(vsgv,"vsgv.csv")
#write.csv(dsgv,"dsgv.csv")
