...
all_basic <- read.table("01.cleanData/phen/Clinical_basic_overlap.tsv",check.names = F)
...
covar1 <- c("read_count","age_bins","gender_num","cohort")
covar2 <- c("read_count","age_bins","gender_num")

### pfs有些数据集只有一套数据有信息，如果校正cohort，取值只有1个，不合理
### os 校正出错
all_resp<-all_basic[,c("response_code","response_code")]
all_pfs<-all_basic[,c("os_2_year","pfs_6_months","pfs_12_months")]

## all
#all_adonis_res <- my_adonis_terms_adjAbun(all_msv_dist_std, all_resp, all_covar, covar1, info)
#save(all_adonis_res, file = "06.Species_genetic_association/RData/all_adonis_res.RData")

#all_adonis_surv <- my_adonis_terms_adjAbun(all_msv_dist_std, all_pfs, all_covar, covar2, info)
#save(all_adonis_surv, file = "06.Species_genetic_association/RData/all_adonis_surv.RData")


### different datasets  (全部为缺失值的变量不能算进去）

covar_PRJEB22863<-PRJEB22863_covar[,c("response_code","pfs_6_months","pfs_12_months")]
PRJEB22863_adonis_res <- my_adonis_terms_adjAbun(PRJEB22863_msv_dist_std, covar_PRJEB22863, PRJEB22863_covar,covar2, info)
save(PRJEB22863_adonis_res, file = "06.Species_genetic_association/RData/PRJEB22863_adonis_res.RData")

covar_PRJEB22863_NSCLC<-PRJEB22863_NSCLC_covar[,c("response_code","pfs_6_months","pfs_12_months")]
PRJEB22863_NSCLC_adonis_res <- my_adonis_terms_adjAbun(PRJEB22863_NSCLC_msv_dist_std, covar_PRJEB22863_NSCLC, PRJEB22863_NSCLC_covar,covar2, info)
save(PRJEB22863_NSCLC_adonis_res, file = "06.Species_genetic_association/RData/PRJEB22863_NSCLC_adonis_res.RData")

covar_PRJEB22863_RCC<-PRJEB22863_RCC_covar[,c("response_code","pfs_6_months","pfs_12_months")]
PRJEB22863_RCC_adonis_res <- my_adonis_terms_adjAbun(PRJEB22863_RCC_msv_dist_std, covar_PRJEB22863_RCC, PRJEB22863_RCC_covar,covar2, info)
...
PRJNA397906_adonis_res <- my_adonis_terms_adjAbun(PRJNA397906_msv_dist_std, covar_PRJNA397906, PRJNA397906_covar, covar2, info)
save(PRJNA397906_adonis_res, file = "06.Species_genetic_association/RData/PRJNA397906_adonis_res.RData")

covar_PRJNA751792<-PRJNA751792_covar[,c("response_code","os_2_year")]
PRJNA751792_adonis_res <- my_adonis_terms_adjAbun(PRJNA751792_msv_dist_std, covar_PRJNA751792, PRJNA751792_covar, covar2, info)
save(PRJNA751792_adonis_res, file = "06.Species_genetic_association/RData/PRJNA751792_adonis_res.RData")

covar_PRJNA762360<-PRJNA762360_covar[,c("response_code","os_2_year","irAEs","pfs_6_months","pfs_12_months")]
PRJNA762360_adonis_res <- my_adonis_terms_adjAbun(PRJNA762360_msv_dist_std, covar_PRJNA762360, PRJNA762360_covar, covar2, info)
save(PRJNA762360_adonis_res, file = "06.Species_genetic_association/RData/PRJNA762360_adonis_res.RData")

covar_PRJNA770295<-PRJNA770295_covar[,c("response_code","pfs_6_months","pfs_12_months")]
PRJNA770295_adonis_res <- my_adonis_terms_adjAbun(PRJNA770295_msv_dist_std, covar_PRJNA770295, PRJNA770295_covar, covar2, info)
...
PRJEB43119_adonis_res <- my_adonis_terms_adjAbun(PRJEB43119_msv_dist_std, covar_PRJEB43119, PRJEB43119_covar, covar2, info)
save(PRJEB43119_adonis_res, file = "06.Species_genetic_association/RData/PRJEB43119_adonis_res.RData")

#### 表型数据不够，PRJNA541981不单独计算
#covar_PRJNA541981<-PRJNA541981_covar[,c("os_2_year","os_2_year")]
#PRJNA541981_adonis_res <- my_adonis_terms_adjAbun(PRJNA541981_msv_dist_std, covar_PRJNA541981, PRJNA541981_covar, covar2, info)
...
colnames(PRJEB22863_adonis_res.table)<-c(paste("PRJEB22863.",colnames(PRJEB22863_adonis_resp)[1:5],sep = ""))
...
colnames(PRJEB22863_NSCLC_adonis_resp)<-c(paste("PRJEB22863_NSCLC.",colnames(PRJEB22863_NSCLC_adonis_resp)[1:5],sep = ""))

PRJEB22863_RCC_adonis_resp<-subset(PRJEB22863_RCC_adonis_res.table,Prog=="response_code",drop=T)
colnames(PRJEB22863_RCC_adonis_resp)<-c(paste("PRJEB22863_RCC.",colnames(PRJEB22863_RCC_adonis_resp)[1:7],sep = ""))
...
colnames(PRJNA397906_adonis_resp)<-c(paste("PRJNA397906.",colnames(PRJNA397906_adonis_resp)[1:5],sep = ""))
...
colnames(PRJNA751792_adonis_resp)<-c(paste("PRJNA751792.",colnames(PRJNA751792_adonis_resp)[1:7],sep = ""))
...
colnames(PRJNA762360_adonis_resp)<-c(paste("PRJNA762360.",colnames(PRJNA762360_adonis_resp)[1:7],sep = ""))
...
colnames(PRJNA770295_adonis_resp)<-c(paste("PRJNA770295.",colnames(PRJNA770295_adonis_resp)[1:5],sep = ""))
...
colnames(PRJEB43119_adonis_resp)<-c(paste("PRJEB43119.",colnames(PRJEB43119_adonis_resp)[1:5],sep = ""))
...
mel_resp_adonis <- my_batch_meta_p(mel_cbind_adonis_resp, c("PRJNA397906","PRJNA762360",
"PRJNA770295","PRJEB43119"),seq(5,22,by=5),seq(4,22,by=5))$table
...
nsclc_resp_adonis <- my_batch_meta_p(nsclc_cbind_adonis_resp, c("PRJNA751792","PRJEB22863_NSCLC"),
...
##########################################################################################################
### os>=2 years and species

PRJNA762360_adonis_os<-subset(PRJNA762360_adonis_res.table,Prog=="os_2_year",drop=T)
colnames(PRJNA762360_adonis_os)<-c(paste("PRJNA762360.",colnames(PRJNA762360_adonis_os)[1:7],sep = ""))
write.csv(PRJNA762360_adonis_os, "06.Species_genetic_association/mel_os_adonis.csv",row.names=F)

PRJNA751792_adonis_os<-subset(PRJNA751792_adonis_res.table,Prog=="os_2_year",drop=T)
colnames(PRJNA751792_adonis_os)<-c(paste("PRJNA751792.",colnames(PRJNA751792_adonis_os)[1:7],sep = ""))
write.csv(PRJNA751792_adonis_os, "06.Species_genetic_association/nsclc_os_adonis.csv",row.names=F)


##########################################################################################################
### pfs  6 months and species

PRJNA762360_adonis_pfs_6<-subset(PRJNA762360_adonis_res.table,Prog=="pfs_6_months",drop=T)
colnames(PRJNA762360_adonis_pfs_6)<-c(paste("PRJNA762360.",colnames(PRJNA762360_adonis_pfs_6)[1:7],sep = ""))

PRJNA770295_adonis_pfs_6<-subset(PRJNA770295_adonis_res.table,Prog=="pfs_6_months",drop=T)
PRJNA770295_adonis_pfs_6<-PRJNA770295_adonis_pfs_6[,-c(1,2)]
colnames(PRJNA770295_adonis_pfs_6)<-c(paste("PRJNA770295.",colnames(PRJNA770295_adonis_pfs_6)[1:5],sep = ""))

mel_cbind_adonis_pfs_6<-cbind(PRJNA762360_adonis_pfs_6,PRJNA770295_adonis_pfs_6)
mel_adonis_res <- my_batch_meta_p(mel_cbind_adonis_pfs_6, c("PRJNA762360","PRJNA7702953_NSCLC"),
seq(5,12,by=5),seq(4,12,by=5))$table
write.csv(mel_adonis_res, "06.Species_genetic_association/mel_pfs_6_months_adonis.csv",row.names=F)

PRJEB22863_NSCLC_adonis_pfs_6<-subset(PRJEB22863_NSCLC_adonis_res.table,Prog=="pfs_6_months",drop=T)
colnames(PRJEB22863_NSCLC_adonis_pfs_6)<-c(paste("PRJEB22863_NSCLC.",colnames(PRJEB22863_NSCLC_adonis_pfs_6)[1:7],sep = ""))
write.csv(PRJEB22863_NSCLC_adonis_pfs_6, "06.Species_genetic_association/nsclc_pfs_6_months_adonis.csv",row.names=F)

PRJEB22863_RCC_adonis_pfs_6<-subset(PRJEB22863_RCC_adonis_res.table,Prog=="pfs_6_months",drop=T)
colnames(PRJEB22863_RCC_adonis_pfs_6)<-c(paste("PRJEB22863_RCC.",colnames(PRJEB22863_RCC_adonis_pfs_6)[1:7],sep = ""))
write.csv(PRJEB22863_RCC_adonis_pfs_6, "06.Species_genetic_association/rcc_pfs_6_months_adonis.csv",row.names=F)

#### irAEs
PRJNA762360_adonis_irAEs<-subset(PRJNA762360_adonis_res.table,Prog=="irAEs",drop=T)
colnames(PRJNA762360_adonis_irAEs)<-c(paste("PRJNA762360.",colnames(PRJNA762360_adonis_pfs_6)[1:7],sep = ""))
...
colnames(PRJNA762360_adonis_pfs_12)<-c(paste("PRJNA762360.",colnames(PRJNA762360_adonis_pfs_12)[1:7],sep = ""))

PRJNA770295_adonis_pfs_12<-subset(PRJNA770295_adonis_res.table,Prog=="pfs_12_months",drop=T)[,-c(1,2)]
colnames(PRJNA770295_adonis_pfs_12)<-c(paste("PRJNA770295.",colnames(PRJNA770295_adonis_pfs_12)[1:5],sep = ""))

PRJEB43119_adonis_pfs_12<-subset(PRJEB43119_adonis_res.table,Prog=="pfs_12_months",drop=T)[,-c(1,2)]
colnames(PRJEB43119_adonis_pfs_12)<-c(paste("PRJEB43119.",colnames(PRJEB43119_adonis_pfs_12)[1:5],sep = ""))

mel_cbind_adonis_pfs_12<-cbind(PRJNA762360_adonis_pfs_12,PRJNA770295_adonis_pfs_12,PRJEB43119_adonis_pfs_12)

mel_adonis_res <- my_batch_meta_p(mel_cbind_adonis_pfs_12, c("PRJNA762360","PRJNA7702953","PRJEB43119"),
seq(5,17,by=5),seq(4,17,by=5))$table
...
covar1 <- c("read_count","age_bins","gender_num","cohort")
covar2 <- c("read_count","age_bins","gender_num")
nc_abun<-ncol(PRJEB22863_abun_clr)

covar_PRJEB22863<-PRJEB22863_covar[,c("response_code","pfs_6_months","pfs_12_months")]
PRJEB22863_sv_abun_lm_res <- lm_btw_mats(covar_PRJEB22863,PRJEB22863_abun_clr[,-nc_abun],PRJEB22863_basic,covar2)
save(PRJEB22863_sv_abun_lm_res, file = "06.Species_genetic_association/RData/PRJEB22863_abun_res.RData")

covar_PRJEB22863_NSCLC<-PRJEB22863_NSCLC_covar[,c("response_code","pfs_6_months","pfs_12_months")]
PRJEB22863_NSCLC_sv_abun_lm_res <- lm_btw_mats(covar_PRJEB22863_NSCLC,PRJEB22863_NSCLC_abun_clr[,-nc_abun],PRJEB22863_NSCLC_basic,covar2)
save(PRJEB22863_NSCLC_sv_abun_lm_res, file = "06.Species_genetic_association/RData/PRJEB22863_NSCLC_abun_res.RData")

covar_PRJEB22863_RCC<-PRJEB22863_RCC_covar[,c("response_code","pfs_6_months","pfs_12_months")]
PRJEB22863_RCC_sv_abun_lm_res <- lm_btw_mats(covar_PRJEB22863_RCC,PRJEB22863_RCC_abun_clr[,-nc_abun],PRJEB22863_RCC_basic,covar2)
save(PRJEB22863_RCC_sv_abun_lm_res, file = "06.Species_genetic_association/RData/PRJEB22863_RCC_abun_res.RData")

covar_PRJNA397906<-PRJNA397906_covar[,c("response_code","response_code")]
PRJNA397906_sv_abun_lm_res <- lm_btw_mats(covar_PRJNA397906,PRJNA397906_abun_clr[,-nc_abun],PRJNA397906_basic,covar2)
save(PRJNA397906_sv_abun_lm_res, file = "06.Species_genetic_association/RData/PRJNA397906_abun_res.RData")

covar_PRJNA751792<-PRJNA751792_covar[,c("response_code","os_2_year")]
PRJNA751792_sv_abun_lm_res <- lm_btw_mats(covar_PRJNA751792,PRJNA751792_abun_clr[,-nc_abun],PRJNA751792_basic,covar2)
save(PRJNA751792_sv_abun_lm_res, file = "06.Species_genetic_association/RData/PRJNA751792_abun_res.RData")

covar_PRJNA762360<-PRJNA762360_covar[,c("response_code","os_2_year","irAEs","pfs_6_months","pfs_12_months")]
PRJNA762360_sv_abun_lm_res <- lm_btw_mats(covar_PRJNA762360,PRJNA762360_abun_clr[,-nc_abun],PRJNA762360_basic,covar2)
save(PRJNA762360_sv_abun_lm_res, file = "06.Species_genetic_association/RData/PRJNA762360_abun_res.RData")

covar_PRJNA770295<-PRJNA770295_covar[,c("response_code","pfs_6_months","pfs_12_months")]
PRJNA770295_sv_abun_lm_res <- lm_btw_mats(covar_PRJNA770295,PRJNA770295_abun_clr[,-nc_abun],PRJNA770295_basic,covar2)
save(PRJNA770295_sv_abun_lm_res, file = "06.Species_genetic_association/RData/PRJNA770295_abun_res.RData")

covar_PRJEB43119<-PRJEB43119_covar[,c("response_code","pfs_12_months")]
PRJEB43119_sv_abun_lm_res <- lm_btw_mats(covar_PRJEB43119,PRJEB43119_abun_clr[,-nc_abun],PRJEB43119_basic,covar2)
save(PRJEB43119_sv_abun_lm_res, file = "06.Species_genetic_association/RData/PRJEB43119_abun_res.RData")


## PRJNA541981_sv_abun_Prog_lm_res <- lm_btw_mats(PRJNA541981_Prog,PRJNA541981_abun_clr[,-nc_abun],PRJNA541981_covar,covar2)
...
colnames(PRJEB22863_sv_abun_resp)<-c(paste("PRJEB22863.",colnames(PRJEB22863_sv_abun_resp)[1:12],sep = ""))
...
colnames(PRJEB22863_NSCLC_sv_abun_resp)<-c(paste("PRJEB22863_NSCLC.",colnames(PRJEB22863_NSCLC_sv_abun_resp)[1:12],sep = ""))

PRJEB22863_RCC_sv_abun_resp<-subset(PRJEB22863_sv_abun.table,Y=="response_code",drop=T)
colnames(PRJEB22863_RCC_sv_abun_resp)<-c(paste("PRJEB22863_RCC.",colnames(PRJEB22863_RCC_sv_abun_resp)[1:14],sep = ""))
...
colnames(PRJNA397906_sv_abun_resp)<-c(paste("PRJNA397906.",colnames(PRJNA397906_sv_abun_resp)[1:12],sep = ""))
...
colnames(PRJNA751792_sv_abun_resp)<-c(paste("PRJNA751792.",colnames(PRJNA751792_sv_abun_resp)[1:14],sep = ""))
...
colnames(PRJNA762360_sv_abun_resp)<-c(paste("PRJNA762360.",colnames(PRJNA762360_sv_abun_resp)[1:14],sep = ""))
...
colnames(PRJNA770295_sv_abun_resp)<-c(paste("PRJNA770295.",colnames(PRJNA770295_sv_abun_resp)[1:12],sep = ""))
...
colnames(PRJEB43119_sv_abun_resp)<-c(paste("PRJEB43119.",colnames(PRJEB43119_sv_abun_resp)[1:12],sep = ""))
...
my_batch_meta_p_lr(mel_cbind_sv_abun_resp,seq(3,50,12),seq(4,50,12),"06.Species_genetic_association/mel_resp_abun.csv")

### nsclc 
nsclc_cbind_sv_abun_resp<-cbind(PRJNA751792_sv_abun_resp,PRJEB22863_NSCLC_sv_abun_resp)
my_batch_meta_p_lr(nsclc_cbind_sv_abun_resp,seq(3,24,12),seq(4,24,12),"06.Species_genetic_association/nsclc_resp_abun.csv")

PRJEB22863_RCC_abun_resp<-subset(PRJEB22863_RCC_sv_abun.table,Y=="response_code",drop=T)
colnames(PRJEB22863_RCC_abun_resp)<-c(paste("PRJEB22863_RCC.",colnames(PRJEB22863_RCC_abun_resp)[1:14],sep = ""))
...
PRJNA762360_abun_os<-subset(PRJNA762360_sv_abun.table,Y=="os_2_year",drop=T)
colnames(PRJNA762360_abun_os)<-c(paste("PRJNA762360.",colnames(PRJNA762360_abun_os)[1:14],sep = ""))
write.csv(PRJNA762360_abun_os, "06.Species_genetic_association/mel_os_abun.csv",row.names=F)

PRJNA751792_abun_os<-subset(PRJNA751792_sv_abun.table,Y=="os_2_year",drop=T)
colnames(PRJNA751792_abun_os)<-c(paste("PRJNA751792.",colnames(PRJNA751792_abun_os)[1:14],sep = ""))
write.csv(PRJNA751792_abun_os, "06.Species_genetic_association/nsclc_os_abun.csv",row.names=F)


##########################################################################################################
### pfs  6 months and species

PRJNA762360_abun_pfs_6<-subset(PRJNA762360_sv_abun.table,Y=="pfs_6_months",drop=T)
colnames(PRJNA762360_abun_pfs_6)<-c(paste("PRJNA762360.",colnames(PRJNA762360_abun_pfs_6)[1:14],sep = ""))

PRJNA770295_abun_pfs_6<-subset(PRJNA770295_sv_abun.table,Y=="pfs_6_months",drop=T)
PRJNA770295_abun_pfs_6<-PRJNA770295_abun_pfs_6[,-c(1,2)]
colnames(PRJNA770295_abun_pfs_6)<-c(paste("PRJNA770295.",colnames(PRJNA770295_abun_pfs_6)[1:12],sep = ""))

mel_cbind_abun_pfs_6<-cbind(PRJNA762360_abun_pfs_6,PRJNA770295_abun_pfs_6)

my_batch_meta_p_lr(mel_cbind_abun_pfs_6,seq(3,26,12),seq(4,26,12),"06.Species_genetic_association/mel_pfs_6_months_abun.csv")


PRJEB22863_NSCLC_abun_pfs_6<-subset(PRJEB22863_NSCLC_sv_abun.table,Y=="pfs_6_months",drop=T)
colnames(PRJEB22863_NSCLC_abun_pfs_6)<-c(paste("PRJEB22863_NSCLC.",colnames(PRJEB22863_NSCLC_abun_pfs_6)[1:14],sep = ""))
write.csv(PRJEB22863_NSCLC_abun_pfs_6, "06.Species_genetic_association/nsclc_pfs_6_months_abun.csv",row.names=F)

PRJEB22863_RCC_abun_pfs_6<-subset(PRJEB22863_RCC_sv_abun.table,Y=="pfs_6_months",drop=T)
colnames(PRJEB22863_RCC_abun_pfs_6)<-c(paste("PRJEB22863_RCC.",colnames(PRJEB22863_RCC_abun_pfs_6)[1:14],sep = ""))
write.csv(PRJEB22863_RCC_abun_pfs_6, "06.Species_genetic_association/rcc_pfs_6_months_abun.csv",row.names=F)
...
colnames(PRJNA762360_abun_pfs_12)<-c(paste("PRJNA762360.",colnames(PRJNA762360_abun_pfs_12)[1:14],sep = ""))

PRJNA770295_abun_pfs_12<-subset(PRJNA770295_sv_abun.table,Y=="pfs_12_months",drop=T)[,-c(1,2)]
colnames(PRJNA770295_abun_pfs_12)<-c(paste("PRJNA770295.",colnames(PRJNA770295_abun_pfs_12)[1:12],sep = ""))

PRJEB43119_abun_pfs_12<-subset(PRJEB43119_sv_abun.table,Y=="pfs_12_months",drop=T)[,-c(1,2)]
colnames(PRJEB43119_abun_pfs_12)<-c(paste("PRJEB43119.",colnames(PRJEB43119_abun_pfs_12)[1:12],sep = ""))

mel_cbind_abun_pfs_12<-cbind(PRJNA762360_abun_pfs_12,PRJNA770295_abun_pfs_12,PRJEB43119_abun_pfs_12)
my_batch_meta_p_lr(mel_cbind_abun_pfs_12,seq(3,38,12),seq(4,38,12),"06.Species_genetic_association/mel_pfs_12_months_abun.csv")

#### irAEs
PRJNA762360_abun_irAEs<-subset(PRJNA762360_sv_abun.table,Y=="irAEs",drop=T)
colnames(PRJNA762360_abun_irAEs)<-c(paste("PRJNA762360.",colnames(PRJNA762360_abun_irAEs)[1:14],sep = ""))
...
mel_os_adonis<-read.csv("06.Species_genetic_association/mel_os_adonis.csv",header=T)
mel_os_adonis<-mel_os_adonis[,c(1,(ncol(mel_os_adonis)-3))]
mel_os_adonis$Prog<-"Melanoma_OS_2_years"
colnames(mel_os_adonis)<-c("Species","p","Prog")

mel_pfs_6_months_adonis<-read.csv("06.Species_genetic_association/mel_pfs_6_months_adonis.csv",header=T)
mel_pfs_6_months_adonis<-mel_pfs_6_months_adonis[,c(1,(ncol(mel_pfs_6_months_adonis)-4))]
mel_pfs_6_months_adonis$Prog<-"Melanoma_PFS_6_months"
colnames(mel_pfs_6_months_adonis)<-c("Species","p","Prog")
...
nsclc_os_adonis<-read.csv("06.Species_genetic_association/nsclc_os_adonis.csv",header=T)
nsclc_os_adonis<-nsclc_os_adonis[,c(1,(ncol(nsclc_os_adonis)-3))]
nsclc_os_adonis$Prog<-"NSCLC_OS_2_years"
colnames(nsclc_os_adonis)<-c("Species","p","Prog")

nsclc_pfs_6_months_adonis<-read.csv("06.Species_genetic_association/nsclc_pfs_6_months_adonis.csv",header=T)
nsclc_pfs_6_months_adonis<-nsclc_pfs_6_months_adonis[,c(1,(ncol(nsclc_pfs_6_months_adonis)-3))]
nsclc_pfs_6_months_adonis$Prog<-"NSCLC_pfs_6_months"
colnames(nsclc_pfs_6_months_adonis)<-c("Species","p","Prog")
...
rcc_pfs_6_months_adonis<-read.csv("06.Species_genetic_association/rcc_pfs_6_months_adonis.csv",header=T)
rcc_pfs_6_months_adonis<-rcc_pfs_6_months_adonis[,c(1,(ncol(rcc_pfs_6_months_adonis)-3))]
rcc_pfs_6_months_adonis$Prog<-"RCC_pfs_6_months"
colnames(rcc_pfs_6_months_adonis)<-c("Species","p","Prog")

all_adonis.table<-rbind(mel_resp_adonis,mel_os_adonis,mel_pfs_12_months_adonis,
mel_irAEs_adonis,nsclc_resp_adonis,nsclc_os_adonis,rcc_resp_adonis)
...
mel_resp_abun<-mel_resp_abun[,c(2,(ncol(mel_resp_abun)-4))]
mel_resp_abun$Prog<-"Melanoma_response"
colnames(mel_resp_abun)<-c("Species","p","Prog")

mel_os_abun<-read.csv("06.Species_genetic_association/mel_os_abun.csv",header=T)
mel_os_abun<-mel_os_abun[,c(2,(ncol(mel_os_abun)-9))]
mel_os_abun$Prog<-"Melanoma_OS_2_years"
colnames(mel_os_abun)<-c("Species","p","Prog")

mel_pfs_6_months_abun<-read.csv("06.Species_genetic_association/mel_pfs_6_months_abun.csv",header=T)
mel_pfs_6_months_abun<-mel_pfs_6_months_abun[,c(2,(ncol(mel_pfs_6_months_abun)-4))]
mel_pfs_6_months_abun$Prog<-"Melanoma_PFS_6_months"
colnames(mel_pfs_6_months_abun)<-c("Species","p","Prog")

mel_pfs_12_months_abun<-read.csv("06.Species_genetic_association/mel_pfs_12_months_abun.csv",header=T)
mel_pfs_12_months_abun<-mel_pfs_12_months_abun[,c(2,(ncol(mel_pfs_12_months_abun)-4))]
...
mel_irAEs_abun<-mel_irAEs_abun[,c(2,(ncol(mel_irAEs_abun)-9))]
...
nsclc_resp_abun<-nsclc_resp_abun[,c(2,(ncol(nsclc_resp_abun)-4))]
nsclc_resp_abun$Prog<-"NSCLC_response"
colnames(nsclc_resp_abun)<-c("Species","p","Prog")

nsclc_os_abun<-read.csv("06.Species_genetic_association/nsclc_os_abun.csv",header=T)
nsclc_os_abun<-nsclc_os_abun[,c(2,(ncol(nsclc_os_abun)-9))]
nsclc_os_abun$Prog<-"NSCLC_OS_2_years"
colnames(nsclc_os_abun)<-c("Species","p","Prog")

nsclc_pfs_6_months_abun<-read.csv("06.Species_genetic_association/nsclc_pfs_6_months_abun.csv",header=T)
nsclc_pfs_6_months_abun<-nsclc_pfs_6_months_abun[,c(2,(ncol(nsclc_pfs_6_months_abun)-9))]
nsclc_pfs_6_months_abun$Prog<-"NSCLC_pfs_6_months"
colnames(nsclc_pfs_6_months_abun)<-c("Species","p","Prog")

rcc_resp_abun<-read.csv("06.Species_genetic_association/rcc_resp_abun.csv",header=T)
rcc_resp_abun<-rcc_resp_abun[,c(2,(ncol(rcc_resp_abun)-9))]
rcc_resp_abun$Prog<-"RCC_response"
colnames(rcc_resp_abun)<-c("Species","p","Prog")

rcc_pfs_6_months_abun<-read.csv("06.Species_genetic_association/rcc_pfs_6_months_abun.csv",header=T)
rcc_pfs_6_months_abun<-rcc_pfs_6_months_abun[,c(2,(ncol(rcc_pfs_6_months_abun)-9))]
rcc_pfs_6_months_abun$Prog<-"RCC_pfs_6_months"
colnames(rcc_pfs_6_months_abun)<-c("Species","p","Prog")

all_abun.table<-rbind(mel_resp_abun,mel_os_abun,mel_pfs_12_months_abun,
mel_irAEs_abun,nsclc_resp_abun,nsclc_os_abun,rcc_resp_abun)
...
tiff(file = "pics/FS2C_species_count.venn.tiff", width =600, height =600, res =300) 
...
species.color[is.na(adonis_res.P)==T&all_abun_res.P<0.05&!is.na(all_abun_res.P)]<- 4   #"grey", genetic assoc
...
#pdf("06.Species_genetic_association/specie_Progs.heatmap.pdf", width = 9, height = 8)
#colnames(species.color.plot)<-c("Response (Melanoma)","pfs >= 12 months (Melanoma)",
#"irAEs (Melanoma)","Response (NSCLC)","os >= 2 years (NSCLC)","Response (RCC)")

colnames(species.color.plot)<-c("Response (Melanoma)","os >= 2 years (Melanoma)","pfs >= 12 months (Melanoma)",
"irAEs (Melanoma)","Response (NSCLC)","os >= 2 years (NSCLC)","Response (RCC)")

tiff(file = "pics/F4A_specie_prognosis.heatmap_N50.tiff", width =2500, height =4200, res =300) 

heatmap.2(species.color.plot, 
          col=colorRampPalette(c("#f9f9f9","#993399","#E18727FF", "#0072B5FF","#666666"))(5), # white,red,yellow,blue
          trace = "none", Rowv = F, Colv = F, dendrogram = "none",
          density.info="none",
          cexCol =1.4, srtCol = 62, cexRow =1.4,
          ...
          key = F,
          lmat=rbind( c(0, 4, 3), c(2, 1, 0 )), lhei = c(0.1, 3.5),lwid=c(1, 7, 4.2 ),key.title = NA,
          margins=c(16,0))
...
p_spe_pcoa<-ggplot(species_dis_input.pcoa,aes(X1,X2, color = as.character(prog_vec_input)))+
  geom_point(size = 2,alpha = 0.8)+ggtitle("Melanoma:\nD.formicigenerans")+
...
tiff(file = "pics/F4C_D.formicigenerans_Mel_response.tiff", width =1200, height =950, res =300) 
print(p_spe_pcoa)
dev.off()
...
species_dis<-PRJEB22863_NSCLC_msv_dist_std[[paste("msv_",info$organism[match(species_short_name,info$Short_name)],sep = "")]]

prog_name<-"response_code"
prog_vec<-nsclc_basic[prog_name] %>% na.omit
...
p_spe_pcoa<-ggplot(species_dis_input.pcoa,aes(X1,X2, color = as.character(prog_vec_input)))+
  geom_point(size = 2,alpha = 0.8)+ggtitle("NSCLC:\nA.muciniphila")+
  xlab(paste("PCo1=",round(species_dis_input_mds$eig[1],digits = 2),"%",sep = ""))+
  ylab(paste("PCo2=",round(species_dis_input_mds$eig[2],digits = 2),"%",sep = ""))+
  scale_color_manual(values=c("#BC3C29FF","#6F99ADFF"))+
  theme(plot.subtitle = element_text(vjust = 1), 
...
tiff(file = "pics/F4C_A.muciniphila_NSCLC_response.tiff", width =1200, height =950, res =300) 
print(p_spe_pcoa)
dev.off()
...
tiff(file = "pics/F4C_R.bromii_NSCLC_response.tiff", width =1200, height =950, res =300) 
print(p_spe_pcoa)
dev.off()
...
#### RCC
species_short_name<-"A.muciniphila"
species_dis<-PRJEB22863_RCC_msv_dist_std[[paste("msv_",info$organism[match(species_short_name,info$Short_name)],sep = "")]]

prog_name<-"response_code"
prog_vec<-rcc_basic[prog_name] %>% na.omit
...
p_spe_pcoa<-ggplot(species_dis_input.pcoa,aes(X1,X2, color = as.character(prog_vec_input)))+
  geom_point(size = 2,alpha = 0.8)+ggtitle("RCC:\nA.muciniphila")+
  xlab(paste("PCo1=",round(species_dis_input_mds$eig[1],digits = 2),"%",sep = ""))+
  ylab(paste("PCo2=",round(species_dis_input_mds$eig[2],digits = 2),"%",sep = ""))+
  scale_color_manual(values=c("#BC3C29FF","#6F99ADFF"))+
  theme(plot.subtitle = element_text(vjust = 1), 
...
tiff(file = "pics/F4C_A.muciniphila_RCC_response.tiff",  width =1200, height =950, res =300) 
print(p_spe_pcoa)
dev.off()
...
prog_vec_input[prog_vec_input==0]<-"OS < 2 years"
prog_vec_input[prog_vec_input==1]<-"OS >= 2 years"
...
p_spe_pcoa<-ggplot(species_dis_input.pcoa,aes(X1,X2, color = as.character(prog_vec_input)))+
  ...
  xlab(paste("PCo1=",round(species_dis_input_mds$eig[1],digits = 2),"%",sep = ""))+
  ylab(paste("PCo2=",round(species_dis_input_mds$eig[2],digits = 2),"%",sep = ""))+
  scale_color_manual(values=c("#FFDC91FF","#EE4C97FF"))+
  theme(plot.subtitle = element_text(vjust = 1), 
        plot.caption = element_text(vjust = 1), 
        axis.line.x =  element_line(),
        axis.line.y = element_line(),
...
tiff(file = "pics/F4F_Melanoma_E.rectale_OS.tiff", width =1200, height =950, res =300) 
print(p_spe_pcoa)
dev.off()
...
tiff(file = "pics/F4F_NSCLC_R.bromii_OS.tiff", width =1200, height =950, res =300) 
print(p_spe_pcoa)
dev.off()
...
tiff(file = "pics/F4F_Melanoma_D.formicigenerans_PFS.tiff", width =1200, height =950, res =300) 
print(p_spe_pcoa)
dev.off()
...
prog_name<-"irAEs"
prog_vec<-all_basic[prog_name] %>% na.omit
...
prog_vec_input[prog_vec_input==0]<-"Non-irAEs"
prog_vec_input[prog_vec_input==1]<-"irAEs"

species_dis_input_mds<-cmdscale(species_dis_input, k=3, eig = T)
species_dis_input.pcoa <- data.frame(species_dis_input_mds$points)
...
p_spe_pcoa<-ggplot(species_dis_input.pcoa,aes(X1,X2, color = as.character(prog_vec_input)))+
  ...
  xlab(paste("PCo1=",round(species_dis_input_mds$eig[1],digits = 2),"%",sep = ""))+
  ylab(paste("PCo2=",round(species_dis_input_mds$eig[2],digits = 2),"%",sep = ""))+
  scale_color_manual(values=c("#999900","#339900"))+
  theme(plot.subtitle = element_text(vjust = 1), 
        plot.caption = element_text(vjust = 1), 
        axis.line.x =  element_line(),
        axis.line.y = element_line(),
...
tiff(file = "pics/F4_Akk_irAEs.tiff", width =1200, height =950, res =300) 
print(p_spe_pcoa)
dev.off()
...
setwd("D:/analysis_lab/pharmacomicrobiomics/1_ICB_metagenome_SNV/pipeline/4_R/06.Species_genetic_association")

species.sig<-read.csv("species.sig.csv",header=T)

Melanoma_irAEs<-subset(species.sig,Prog=="Melanoma_irAEs",drop=T)
Melanoma_OS_2_years<-subset(species.sig,Prog=="Melanoma_OS_2_years",drop=T)
...
nsclc_os<-subset(species.sig,Prog=="NSCLC_OS_2_years",drop=T)
...
mel_irAEs_abun$Species<-info$Short_name[match(mel_irAEs_abun$PRJNA762360.X, info$organism)]
...
mel_OS_2_years_adonis<-read.csv("mel_os_adonis.csv",header=T)
mel_OS_2_years_abun<-read.csv("mel_os_abun.csv",header=T)
colnames(mel_OS_2_years_adonis)[1]<-"Species"
mel_OS_2_years_abun$Species<-info$Short_name[match(mel_OS_2_years_abun$PRJNA762360.X, info$organism)]

Melanoma_OS_2_years_all<-merge(Melanoma_OS_2_years,mel_OS_2_years_adonis,by.x="Species",by.y="Species")
Melanoma_OS_2_years_all<-merge(Melanoma_OS_2_years_all,mel_OS_2_years_abun,by.x="Species",by.y="Species")

write.csv(Melanoma_OS_2_years_all,"table/Melanoma_os_all.csv",row.names=F)
...
mel_pfs_12_months_abun$Species<-info$Short_name[match(mel_pfs_12_months_abun$PRJNA762360.X, info$organism)]
...
mel_response_abun$Species<-info$Short_name[match(mel_response_abun$PRJNA762360.X, info$organism)]
...
########################### NSCLC_os
nsclc_os_adonis<-read.csv("nsclc_os_adonis.csv",header=T)
nsclc_os_abun<-read.csv("nsclc_os_abun.csv",header=T)
colnames(nsclc_os_adonis)[1]<-"Species"
nsclc_os_abun$Species<-info$Short_name[match(nsclc_os_abun$PRJNA751792.X, info$organism)]

nsclc_os_all<-merge(nsclc_os,nsclc_os_adonis,by.x="Species",by.y="Species")
nsclc_os_all<-merge(nsclc_os_all,nsclc_os_abun,by.x="Species",by.y="Species")

write.csv(nsclc_os_all,"table/nsclc_os_all.csv",row.names=F)
...
nsclc_response_abun$Species<-info$Short_name[match(nsclc_response_abun$PRJNA751792.X, info$organism)]
...
rcc_response_abun$Species<-info$Short_name[match(rcc_response_abun$PRJEB22863_RCC.X, info$organism)]
