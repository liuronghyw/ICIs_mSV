...
my_adonis_terms<-function(inDist, inDf, covDf=NULL, covar=NULL){
  ...
  my_adonis<-function(inMat, inVec, covdf = NULL){
    ...
    if(length(table(inVec_rmna))<2){
      ...
      adonis_res <- adonis(as.dist(inMat_rmna)~.,data = in_cov,parallel = 2)
...
logistic_btw_mats_adjAbun_dsv<-function(y_mat, x_mat, cov_mat, covar, abun, info, abun_col,pheno){
  ...
  ############# test block  
  #y_mat<-PRJEB22863_prog
  #x_mat<-PRJEB22863_RCC_dsgv
  #cov_mat<-PRJEB22863_RCC_basic
  #covar<-PRJEB22863_covar
  #abun<-PRJEB22863_RCC_abun_clr
  #info<-info
  #abun_col<-1
  #pheno<-"response_code"
  ############# test block
...
logistic_btw_mats_dsv<-function(y_mat,x_mat,cov_mat,covar,pheno){
  ...
  my_lg<-function(pheno,x){
   ...
   # x<-x_mat_i[,2]
    ...
    if(nrow(lg_input)<=30 | min(table(lg_input$Y))<5 | length(table(lg_input$Y))==1){
  ...
  colnames(y_x_edge)<-c("X", "OR","SE", "p","OR_left","OR_right","N","x_uniq_N", "x_non_zero_N", "x_non_zero_rate","fdr.p","bonferroni.p")
  return(y_x_edge)
...
cox_btw_mats_dsv<-function(y_mat,x_mat,cov_mat,covar,surv_time,surv_event){
   ...
   # x<-x_mat[,1]
    ...
    cox_input<-as.data.frame(cox_input)
     ...
     if(is.null(nrow(cox_input))==T){
      ...
      else if (is.null(nrow(cox_input))==F & nrow(cox_input)<=30){
        try(return(list(hr =NA,
                    se = NA,
                    p.value = NA,
                    hr_left=NA,
  ...
  colnames(y_x_edge)<-c("X", "HR","SE", "p","HR_left","HR_right","N","x_uniq_N", "x_non_zero_N", "x_non_zero_rate","fdr.p","bonferroni.p")
  return(y_x_edge)
}
...
#i=944
  ...
  for(i in 1:nr){
    ...
    else if (length(ind)>0 & length(ind)<=((nc-1)/2-2)){  
   ...
   else if (length(ind)==(nc-1)/2 | length(ind)==(nc-1)/2-1){ 
      effect_meta[i]<-NA
      pvalue_meta[i]<-NA
      cilb[i]<-NA
      ciub[i]<-NA
...
combine_meta_p_lr<-function(datain,dataout){
    #data<-cbind_dsv_res.table
     data<-datain
     nc<-ncol(data)
     nr<-nrow(data)
     effect<-data[,seq(2,nc,2)]
     se<-data[,seq(3,nc,2)]
...
multi_adjust<-function(datain,dataout){
   ...
   #p_test<-read.csv("08.Microbial_GWAS/all_vsv_meta_pvalue.csv",header=T)
  ...
  re_out<-cbind(re_rnma,fdr_all[-1],bonferroni_all[-1])
